...
> [!NOTE]
> 实验目标是让管理器分别记住每个 Mod 的服装部件、颜色、变体和其他内部 `$变量`，并在切回 Mod 或应用组合方案时自动恢复。下列原型均基于未修改的 v3.8.0 正式基线建立在独立分支中，没有直接替换正式版。

| 实验版本 | 研究方法 | 实际结果 |
| --- | --- | --- |
| v3.8.1 Hot Injection Lab | 完整保存并热替换 `d3dx_user.ini`，随后请求 F10 重载 | 共享文件会同时覆盖其他 Mod 和加载器状态；F10 可能先把旧内存值写回文件，恢复结果不可靠 |
| v3.8.1 Persistent State Lab | 解析并合并 `global persist` 标量；另行测试临时桥接 INI 和两次手动 F10 | 只能覆盖部分数值变量，无法通用处理纯内存状态、复杂类型、后续命令覆盖和不同加载器时序 |
| v3.8.2 EFMI Resident Lab | 生成常驻受控 Mod 副本，以 `$active_profile` 切换方案 | 只适用于能够证明完整门控的标准结构；任意第三方 Mod 兼容性、性能和真实游戏验证无法保证 |
| v3.8.3—v3.8.5 目录联接 | 让加载器和仓库引用同一个 Mod 目录 | 对 Mod 文件夹内部实际落盘的配置有效；不覆盖 EFMI 根目录的 `d3dx_user.ini` 或仅存在于游戏内存的状态 |

> [!WARNING]
> **当前结论：对任意第三方 XXMI / EFMI Mod 的内部 `$变量` 进行通用、自动、可靠的跨 Mod 保存与恢复，现阶段暂时无法实现。** v3.8.1 和 v3.8.2 原型均未发布，也不包含在 v3.8.5 安装包中。

v3.8.5 保留可选的目录联接部署和原有复制模式。目录联接可以解决“加载器修改了部署副本，但仓库原 Mod 没有同步”的问题；同一角色链接另一个 Mod 时会安全断开之前的受管联接。它不读取或写入游戏进程内存，不执行 DLL 注入、自动 F10 或反作弊绕过。

[查看完整实验过程、失败原因、支持边界与重新评估条件](./docs/跨Mod状态保存试验结论.md)
