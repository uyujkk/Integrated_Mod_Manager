...
public sealed partial class MainWindow : Window
{
    ...
    [
        ...
        new("Zhuang Fangyi", "庄方宜", "44984", "https://bbs.hycdn.cn/image/common/20260417/2641152/69e12fbefe4a20e7c0d83e4e_f1525633.png", "Fangyi", "ZFY", "庄方怡"),
    ...
    private string? _onlineCacheStatusZh;
    private string? _onlineCacheStatusEn;
    private string? _lastLoadedOnlineConfigKey;
    private string? _blockedOnlineAutoLoadConfigKey;
    ...
    private int _onlineCharacterAvatarRequestVersion;
    private CancellationTokenSource _onlineCharacterAvatarCancellation = new();
    ...
    private void RefreshProjectResourceText()
    {
        ProjectResourcesTitleTextBlock.Text = L("项目与开源组件", "Project and Open-Source Components");
        ProjectResourcesHintTextBlock.Text = L(
            "列出软件运行时使用或调用的主要开源项目，以及本项目和作者主页。点击任意项目可打开其官方地址。",
            "Major open-source projects used or invoked at runtime, plus this project and the author's homepage. Select an item to open its official address.");

        foreach (Button resourceButton in ProjectResourcesGrid.Children.OfType<Button>())
        {
            if (resourceButton.Tag is not string url || string.IsNullOrWhiteSpace(url))
            {
                continue;
            }

            ToolTipService.SetToolTip(resourceButton, url);
            string resourceName = resourceButton.Content is StackPanel panel
                ? panel.Children.OfType<TextBlock>().FirstOrDefault()?.Text ?? url
                : url;
            AutomationProperties.SetName(resourceButton, $"{resourceName} · {url}");
        }
    }
    ...
    private void RefreshUpdatesPane()
    {
        ...
        InstallSafetyHintTextBlock.Text = L("安装前检查文件冲突；复制、移除和方案切换均可通过备份恢复。", "Check file conflicts before installation; copies, removals, and profile changes can be recovered from backups.");
        ...
        DownloadTaskCenterTitleTextBlock.Text = L("下载任务", "Downloads");
        DownloadTaskCenterHintTextBlock.Text = L("显示在线 Mod 的下载与解压进度。", "Shows online mod download and extraction progress.");
        ...
        TrackedModSettingsHintTextBlock.Text = L("按设定频率在启动时检查，也可立即刷新当前仓库。", "Check on startup at the selected interval, or refresh this repository now.");
        ModUpdateIntervalLabelTextBlock.Text = L("检查频率", "Check interval");
        ModUpdateIntervalHintTextBlock.Text = L("仅检查已保存来源 ID 的在线安装 Mod。", "Only online-installed mods with a saved source ID are checked.");
    ...
    private void ApplyTrackedModsListLayout()
    {
        ...
        int columnCount = _useWideTrackedModsLayout ? 2 : 1;
        TrackedModsPrimaryColumn.Width = new GridLength(1, GridUnitType.Star);
        TrackedModsSecondaryColumn.Width = _useWideTrackedModsLayout
        ...
        {
            FrameworkElement child = (FrameworkElement)TrackedModsListPanel.Children[index];
            Grid.SetRow(child, index / columnCount);
            Grid.SetColumn(child, index % columnCount);
            Grid.SetColumnSpan(child, childCount == 1 && columnCount == 2 ? 2 : 1);
    ...
    private void LoadShellConfig()
    {
        ...
        try
        {
            string shellJson = ReadConfigurationTextWithRecovery(_shellConfigPath, IsValidShellConfiguration);
            BetaShellConfig? config = JsonSerializer.Deserialize<BetaShellConfig>(shellJson);
    ...
    private void SaveShellConfig()
    {
        try
        {
            ...
            WriteConfigurationTextWithBackup(_shellConfigPath, json);
        }
        catch
        {
    ...
    private async void ShowAppNotification(string zh, string en, InfoBarSeverity severity = InfoBarSeverity.Success)
    {
        ...
        AppInfoBar.Title = severity switch
        {
            InfoBarSeverity.Error => L("操作未完成", "Action not completed"),
            InfoBarSeverity.Warning => L("访问已停止", "Stopped"),
            InfoBarSeverity.Informational => L("提示", "Notice"),
            _ => L("操作完成", "Completed")
        };
    ...
    private void RefreshOnlineCharacterAvatarStrip()
    {
        ...
        _onlineCharacterAvatarCancellation = new CancellationTokenSource();
        CancellationToken avatarCancellationToken = _onlineCharacterAvatarCancellation.Token;
        int avatarRequestVersion = ++_onlineCharacterAvatarRequestVersion;
    ...
    private FrameworkElement CreateOnlineCharacterButton(
        ...
        string? avatarUrl,
        int? modCount,
        int avatarRequestVersion,
        CancellationToken avatarCancellationToken)
    {
        ...
        {
            var avatarLayer = new Grid();
            avatarLayer.Children.Add(new FontIcon
            {
                Glyph = "\uE77B",
                FontSize = 22,
                HorizontalAlignment = HorizontalAlignment.Center,
    ...
    private void RefreshOnlinePaneV2()
    {
        ...
        bool hasCacheStatus = !string.IsNullOrWhiteSpace(_onlineCacheStatusZh)
            && !string.IsNullOrWhiteSpace(_onlineCacheStatusEn);
        OnlineCacheStatusBorder.Visibility = hasCacheStatus ? Visibility.Visible : Visibility.Collapsed;
        OnlineCacheStatusTextBlock.Text = hasCacheStatus
            ? L(_onlineCacheStatusZh!, _onlineCacheStatusEn!)
            : string.Empty;
        ...
        _onlineVisibleMods.Clear();
        OnlineEmptyResultsContentControl.Content = null;
        OnlineEmptyResultsContentControl.Visibility = Visibility.Collapsed;

        if (!hasRepository)
        {
            ShowOnlineEmptyResults(CreateOnlinePreviewCard(
        ...
        {
            DispatcherQueue.TryEnqueue(() => UpdateOnlineGridCardMetrics(
                GetOnlineGridAvailableWidth(OnlineVirtualizedGridView.ActualWidth)));
        }
        DispatcherQueue.TryEnqueue(UpdateOnlineWorkspaceWidthConstraint);
        AnimateOnlineCardsEntrance();
        ...
        if (_currentPrimarySection == PrimarySection.Online
            && canLoadOnlineMods
            && configKey != _lastLoadedOnlineConfigKey
            && !string.Equals(configKey, _blockedOnlineAutoLoadConfigKey, StringComparison.Ordinal)
            && !_isLoadingOnlineMods)
        {
    ...
    private void ShowOnlineEmptyResults(UIElement content)
    {
        OnlineEmptyResultsContentControl.Content = content;
        OnlineEmptyResultsContentControl.Visibility = Visibility.Visible;
    }
    ...
    private void SetOnlineCacheStatus(string? zh, string? en)
    {
        _onlineCacheStatusZh = zh;
        _onlineCacheStatusEn = en;
        bool hasStatus = !string.IsNullOrWhiteSpace(zh) && !string.IsNullOrWhiteSpace(en);
        OnlineCacheStatusBorder.Visibility = hasStatus ? Visibility.Visible : Visibility.Collapsed;
        OnlineCacheStatusTextBlock.Text = hasStatus ? L(zh!, en!) : string.Empty;
        ToolTipService.SetToolTip(OnlineCacheStatusBorder, hasStatus ? L(zh!, en!) : null);
    }
    ...
    private (string Zh, string En) FormatOnlineCacheAge(DateTimeOffset? cachedAtUtc, bool stale)
    {
        ...
        {
            return stale ? ("离线缓存", "Stale cache") : ("本地缓存", "Local cache");
        }

        TimeSpan age = DateTimeOffset.UtcNow - cachedAtUtc.Value;
        string valueZh;
        string valueEn;
        ...
        return stale
            ? ($"离线缓存 · {valueZh}", $"Stale cache · {valueEn}")
    ...
    private async void OnRefreshOnlineModsClicked(object sender, RoutedEventArgs e)
    {
        ...
        {
            _ = EnsureOnlineCharacterCatalogAsync(repository, forceReload: true);
        }
    ...
    private async Task LoadOnlineModsAsync(bool forceReload)
    {
        ...
        else if (string.Equals(configKey, _blockedOnlineAutoLoadConfigKey, StringComparison.Ordinal))
        {
            RefreshOnlinePaneV2();
            return;
        }
        ...
        _onlineModListCancellation = requestCancellation;
        int requestVersion = ++_onlineModRequestVersion;
        ...
        SetOnlineCacheStatus("正在连接", "Connecting");
        RefreshOnlinePaneV2();
        using CancellationTokenSource timeoutCancellation = new(OnlineModListRequestTimeout);
        ...
        try
        {
            ...
            bool loadedAfterTimeout = false;
            OnlineCategoryPageResult pageResult;
            if (cachedPage is null && TryGetOnlineRateLimitDelay(out TimeSpan cooldownRemaining))
            {
                ...
                {
                    int seconds = Math.Max(1, (int)Math.Ceiling(cooldownRemaining.TotalSeconds));
                    _blockedOnlineAutoLoadConfigKey = configKey;
                    SetOnlineStatus(
                        $"访问频率受限，{seconds} 秒后可再次刷新；当前内容保持不变。",
                        $"Rate limit cooldown is active. Try again in {seconds} seconds; current content is preserved.");
                    SetOnlineCacheStatus($"冷却 {seconds} 秒", $"Cooldown {seconds}s");
                    return;
                }
                ...
                try
                {
                    pageResult = cachedPage
                        ?? (needsCharacterFallback
                            ? await FetchGameBananaCharacterFallbackPageAsync(
                                baseCategoryId,
                                _onlineCharacterFilter,
                                _onlineCurrentPage,
                                useEndfieldMetadata,
                                linkedCancellation.Token)
                            : await FetchGameBananaCategoryPageAsync(
                                categoryId,
                                _onlineCurrentPage,
                    ...
                    if (!loadedFromCache)
                    {
                        ResetOnlineRateLimit();
                    }
                }
                catch (OnlineRateLimitException ex)
                {
                    RegisterOnlineRateLimit(ex.RetryAfter);
                    OnlineCategoryPageResult? stalePage = await TryReadOnlineCategoryPageCacheAsync(
                        cacheCategoryKey,
                        _onlineCurrentPage,
                        allowExpired: true);
            ...
            (string cacheZh, string cacheEn) = loadedFromCache
                ? FormatOnlineCacheAge(pageResult.CachedAtUtc, loadedDuringRateLimit || loadedAfterTimeout)
                : ("实时数据", "Live");
            SetOnlineCacheStatus(cacheZh, cacheEn);
            SetOnlineStatus(
                loadedAfterTimeout
                    ? $"连接超过 5 秒，已停止访问并显示本地缓存 · {roleLabelZh} · 第 {_onlineCurrentPage} 页"
                ...
                loadedAfterTimeout
                    ? $"Connection exceeded 5 seconds; stopped and showing local cache · {roleLabelEn} · Page {_onlineCurrentPage}"
                    : loadedDuringRateLimit
            ...
            {
                ShowAppNotification(
                    "在线访问超过 5 秒，已停止并显示本地缓存。",
                    "Online access exceeded 5 seconds; the request was stopped and local cache is shown.",
                    InfoBarSeverity.Warning);
            }
            else if (loadedDuringRateLimit)
            {
                ShowAppNotification(
        ...
        catch (OnlineRateLimitException)
        {
            ...
            SetOnlineStatus(
                $"GameBanana 请求较频繁，已暂停访问；{seconds} 秒后可再次刷新。",
                $"GameBanana is rate limiting requests. Access is paused; try again in {seconds} seconds.");
            SetOnlineCacheStatus($"冷却 {seconds} 秒", $"Cooldown {seconds}s");
            ShowAppNotification(
        ...
        catch (OperationCanceledException) when (timeoutCancellation.IsCancellationRequested)
        {
            ...
            SetOnlineStatus(
                ...
                "Connection exceeded 5 seconds. The request was stopped and current content was preserved.");
            SetOnlineCacheStatus("访问已停止", "Stopped");
            ShowAppNotification(
                "在线 Mod 访问超时，已在 5 秒后停止。",
                "Online mod access timed out and was stopped after 5 seconds.",
                InfoBarSeverity.Warning);
        }
        catch (Exception ex)
        {
        ...
        finally
        {
            if (requestVersion == _onlineModRequestVersion)
            {
                ...
                if (ReferenceEquals(_onlineModListCancellation, requestCancellation))
                {
                    _onlineModListCancellation = null;
                }
                RefreshOnlinePaneV2();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new OnlineRateLimitException(response.Headers.RetryAfter?.Delta);
        }
    ...
    private static OnlineModCard MergeOnlineModCard(OnlineModCard baseCard, OnlineModCard? enrichedCard)
    {
        ...
        baseCard.DownloadFileName = !string.IsNullOrWhiteSpace(enrichedCard.DownloadFileName)
            ? enrichedCard.DownloadFileName
            : baseCard.DownloadFileName;
        if (enrichedCard.DownloadFiles.Count > 0)
        {
            baseCard.DownloadFiles = enrichedCard.DownloadFiles;
        }
    ...
    private async Task AnimateOnlineListExitAsync()
    {
        ...
        if (_reduceMotion || _onlineVisibleMods.Count == 0)
        {
            visual.Opacity = 1f;
            return;
        }
    ...
    private async Task<OnlineCharacterCatalogCacheSnapshot?> TryReadOnlineCharacterCatalogCacheAsync(string catalogKey)
    {
        ...
        try
        {
            ...
            return entry?.Characters is { Count: > 0 } characters
                ? new OnlineCharacterCatalogCacheSnapshot(
                    characters,
                    checkedAt,
                    DateTimeOffset.UtcNow - checkedAt <= TimeSpan.FromDays(30))
                : null;
        }
        catch
        {
            ...
            if (!forceReload)
            {
                ...
                if (cachedCatalog is not null)
                {
                    ...
                    if (!requiresWikiCatalog || cachedCatalog.Characters.Any(character => character.IsWikiCharacter))
                    {
                        ...
                        ApplyOnlineCharacterCatalog(catalogKey, cachedCatalog.Characters);
    ...
    private FrameworkElement GetOnlineResultsElement()
    {
        return _onlineCardLayoutMode == OnlineCardLayoutMode.Grid
            ? OnlineVirtualizedGridView
            : OnlineVirtualizedListView;
    }

    private void UpdateOnlineCardLayoutControls()
    {
        bool showGrid = _onlineCardLayoutMode == OnlineCardLayoutMode.Grid;
        OnlineVirtualizedListView.Visibility = showGrid ? Visibility.Collapsed : Visibility.Visible;
        OnlineVirtualizedGridView.Visibility = showGrid ? Visibility.Visible : Visibility.Collapsed;
    ...
    private void OnOnlineListCardPresenterDataContextChanged(FrameworkElement sender, DataContextChangedEventArgs args)
    {
        if (sender is ContentControl presenter)
        {
            presenter.Content = args.NewValue is OnlineModCard mod
                ? CreateOnlineModCardV2(mod)
                : null;
        }
    }

    private void OnOnlineGridCardPresenterDataContextChanged(FrameworkElement sender, DataContextChangedEventArgs args)
    {
        if (sender is ContentControl presenter)
        {
            presenter.Content = args.NewValue is OnlineModCard mod
    ...
    private void OnOnlineResultsHostSizeChanged(object sender, SizeChangedEventArgs e)
    {
        if (_onlineCardLayoutMode == OnlineCardLayoutMode.Grid)
        {
            UpdateOnlineGridCardMetrics(GetOnlineGridAvailableWidth(e.NewSize.Width));
        }
    }

    private void OnOnlineWorkspaceGridSizeChanged(object sender, SizeChangedEventArgs e)
    {
        UpdateOnlineWorkspaceWidthConstraint();
    }

    private void UpdateOnlineWorkspaceWidthConstraint()
    {
    ...
    private double GetOnlineGridAvailableWidth(double fallbackWidth)
    {
        double resultsWidth = OnlineResultsHost?.ActualWidth ?? 0;
        double availableWidth = resultsWidth > 0 ? resultsWidth : fallbackWidth;
        return Math.Max(0, availableWidth - 14);
    }
    ...
    private void UpdateOnlineGridCardMetrics(double availableWidth)
    {
        ...
        if (OnlineVirtualizedGridView.ItemsPanelRoot is not ItemsWrapGrid itemsPanel)
        {
            return;
        }

        if (Math.Abs(itemsPanel.ItemWidth - itemWidth) > 0.5)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        finally
        {
            if (enteredGate)
            {
                _onlineCharacterAvatarDownloadGate.Release();
            }
        }
    }
    ...
    private Uri? TryGetExistingCachedOnlineImageUri(string imageUrl)
    {
    ...
    private string GetOnlineImageCacheFilePath(Uri remoteUri, string imageUrl)
    {
        ...
        return Path.Combine(_onlineImageCachePath, hash + extension);
    }
    ...
    private async Task<byte[]?> DownloadOnlineImageBytesWithRetryAsync(
        Uri remoteUri,
        CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++)
        {
            ...
            {
                if (response.Content.Headers.ContentLength is > 26214400)
                {
                    return null;
                }
            ...
            TimeSpan retryDelay = response.Headers.RetryAfter?.Delta
                ?? TimeSpan.FromMilliseconds(600 * Math.Pow(2, attempt));
            if (retryDelay > TimeSpan.FromSeconds(6))
            {
    ...
    private void UpdateOnlineDetailDownloadControls(OnlineModCard mod)
    {
        ...
        OpenOnlineDetailPageButton.Content = L("打开原页面", "Open Original Page");
        SelectOnlineDetailDownloadButton.Content = availableFileCount > 0
        ...
        if (defaultFile is not null)
        {
            string size = defaultFile.FileSizeBytes > 0 ? $" · {FormatFileSize(defaultFile.FileSizeBytes)}" : string.Empty;
            string date = defaultFile.AddedAt > DateTimeOffset.MinValue
                ? $" · {defaultFile.AddedAt:yyyy-MM-dd}"
                : string.Empty;
            OnlineDetailDownloadSelectionTextBlock.Text = L(
                $"默认：{defaultFile.FileName}{size}{date}",
                $"Default: {defaultFile.FileName}{size}{date}");
            ToolTipService.SetToolTip(OnlineDetailDownloadSelectionTextBlock, OnlineDetailDownloadSelectionTextBlock.Text);
        }
        else if (!string.IsNullOrWhiteSpace(mod.DownloadUrl) || mod.ItemId > 0)
        {
            OnlineDetailDownloadSelectionTextBlock.Text = L(
                "默认下载最新的可用压缩包；可手动选择其他文件。",
                "The newest available archive is downloaded by default; another file can be selected manually.");
            ToolTipService.SetToolTip(OnlineDetailDownloadSelectionTextBlock, null);
        }
        else
        {
            OnlineDetailDownloadSelectionTextBlock.Text = L("当前没有可用下载文件。", "No downloadable file is currently available.");
            ToolTipService.SetToolTip(OnlineDetailDownloadSelectionTextBlock, null);
        }
    }
    ...
    private void ResetOnlineDetailPaneToPlaceholder()
    {
        ...
        DownloadOnlineDetailButton.IsEnabled = false;
        OpenOnlineDetailPageButton.Content = L("打开原页面", "Open Original Page");
        SelectOnlineDetailDownloadButton.Content = L("选择文件", "Select File");
        DownloadOnlineDetailButton.Content = L("下载最新压缩包", "Download Latest Archive");
        OnlineDetailDownloadSelectionTextBlock.Text = L(
            "默认下载最新的可用压缩包；也可手动选择文件。",
            "The newest available archive is downloaded by default; manual file selection is also available.");
        ToolTipService.SetToolTip(OnlineDetailDownloadSelectionTextBlock, null);
        PopulateOnlineDetailImages([]);
    }
    ...
    private UIElement CreateOnlineModTile(OnlineModCard mod)
    {
        ...
        Grid characterContent = new();
        characterContent.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(3) });
        characterContent.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        characterContent.Children.Add(new Border
        {
            Width = 3,
            Background = GetAppThemeBrush("AppNavSelectedBorderBrush")
        });
        ...
        Grid.SetColumn(characterText, 1);
        characterContent.Children.Add(characterText);

        var characterBanner = new Border
        {
            Height = 34,
            HorizontalAlignment = HorizontalAlignment.Stretch,
            VerticalAlignment = VerticalAlignment.Bottom,
            Background = new SolidColorBrush(ColorHelper.FromArgb(190, 12, 17, 25)),
            Child = characterContent
        };
        ToolTipService.SetToolTip(characterBanner, characterLabel);
        imageLayer.Children.Add(characterBanner);

        Border heatBadge = CreateOnlineHeatIndicator(mod.HotnessScore);
    ...
    private Border CreateOnlineHeatIndicator(double hotnessScore)
    {
        ...
        content.Children.Add(new TextBlock
        {
            Text = hotnessScore.ToString("F1", CultureInfo.CurrentCulture),
            FontSize = 12,
            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
            Foreground = new SolidColorBrush(Colors.White),
            VerticalAlignment = VerticalAlignment.Center
        ...
        {
            Background = new SolidColorBrush(ColorHelper.FromArgb(210, 24, 37, 55)),
        ...
        ToolTipService.SetToolTip(indicator, $"{L("热度", "Hotness")}: {hotnessScore:F1}");
        return indicator;
    }
    ...
    private async Task<OnlineModCard> RefreshOnlineModDownloadInfoAsync(OnlineModCard mod)
    {
        try
        {
            ...
            latestMod.Title = string.IsNullOrWhiteSpace(latestMod.Title) ? mod.Title : latestMod.Title;
            if (IsGenericOnlineCharacterName(latestMod.CharacterName)
                && !IsGenericOnlineCharacterName(mod.CharacterName))
            {
                latestMod.CharacterName = mod.CharacterName;
                latestMod.IsCharacterNameInferred = mod.IsCharacterNameInferred;
            }
            else if (string.IsNullOrWhiteSpace(latestMod.CharacterName))
            {
                latestMod.CharacterName = mod.CharacterName;
            }
            latestMod.RootCategoryName = string.IsNullOrWhiteSpace(latestMod.RootCategoryName) ? mod.RootCategoryName : latestMod.RootCategoryName;
            latestMod.Author = string.IsNullOrWhiteSpace(latestMod.Author) ? mod.Author : latestMod.Author;
            latestMod.ProfileUrl = string.IsNullOrWhiteSpace(latestMod.ProfileUrl) ? mod.ProfileUrl : latestMod.ProfileUrl;
            latestMod.PreviewUrl = string.IsNullOrWhiteSpace(latestMod.PreviewUrl) ? mod.PreviewUrl : latestMod.PreviewUrl;
    ...
    private static OnlineModCard WithSelectedDownloadFile(OnlineModCard mod, OnlineDownloadCandidate selectedFile)
    {
        return new OnlineModCard
        {
            ItemId = mod.ItemId,
            ...
            CategoryId = mod.CategoryId,
            IsCharacterNameInferred = mod.IsCharacterNameInferred,
            RootCategoryName = mod.RootCategoryName,
            Author = mod.Author,
            Likes = mod.Likes,
            Views = mod.Views,
            Downloads = mod.Downloads,
            HotnessScore = mod.HotnessScore,
        ...
        bool refreshDownloadInfo = true)
    {
        ...
        OnlineDownloadCandidate? effectiveFile = selectedFile is null
            ...
            : effectiveMod.DownloadFiles.FirstOrDefault(file =>
                !string.IsNullOrWhiteSpace(selectedFile.FileId)
                && string.Equals(file.FileId, selectedFile.FileId, StringComparison.Ordinal))
                ?? selectedFile;
        ...
        OnlineModDetails? details)
    {
        ...
        try
        {
            ...
            List<(string Url, bool IsFallbackThumbnail)> candidateUrls = (details?.ImageUrls ?? [])
                .Where(url => !string.IsNullOrWhiteSpace(url)
                    && !string.Equals(url, mod.PreviewUrl, StringComparison.OrdinalIgnoreCase))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .Take(5)
                .Select(url => (url, false))
                .ToList();
            if (!string.IsNullOrWhiteSpace(mod.PreviewUrl)
                && !candidateUrls.Any(candidate => string.Equals(
                    candidate.Url,
                    mod.PreviewUrl,
                    StringComparison.OrdinalIgnoreCase)))
            {
                candidateUrls.Add((mod.PreviewUrl, true));
            }
            ...
            {
                (string imageUrl, bool isFallbackThumbnail) = candidateUrls[index];
                ...
                byte[]? imageBytes = await GetOnlinePreviewCandidateBytesAsync(
                    imageUrl,
                    imageUri,
                    timeout.Token);
                if (imageBytes is null || imageBytes.Length == 0 || imageBytes.Length > MaxOnlinePreviewImageBytes)
                {
                    continue;
                }

                (int width, int height) = await TryReadOnlineImageDimensionsAsync(imageBytes);
                if (width <= 0 || height <= 0)
                {
                    continue;
                }

                downloadedImages.Add((new OnlinePreviewCandidate
                {
                    Url = imageUrl,
                    PixelWidth = width,
                    PixelHeight = height,
                    FileSizeBytes = imageBytes.LongLength,
                    IsFallbackThumbnail = isFallbackThumbnail,
                    SourceOrder = index
                }, imageBytes));
            }

            OnlinePreviewCandidate? selected = OnlinePreviewSelectionPolicy.SelectBest(
                downloadedImages.Select(item => item.Candidate));
            if (selected is null)
            {
                return null;
            }

            byte[] selectedBytes = downloadedImages.First(item => ReferenceEquals(item.Candidate, selected)).Bytes;
            Uri selectedUri = new(selected.Url);
            string extension = GetOnlinePreviewImageExtension(selectedUri, selectedBytes);
    ...
    private async Task<byte[]?> GetOnlinePreviewCandidateBytesAsync(
        string imageUrl,
        Uri imageUri,
        CancellationToken cancellationToken)
    {
        Uri? cachedUri = TryGetExistingCachedOnlineImageUri(imageUrl);
        if (cachedUri is { IsFile: true } && File.Exists(cachedUri.LocalPath))
        {
            FileInfo cachedFile = new(cachedUri.LocalPath);
            if (cachedFile.Length > 0 && cachedFile.Length <= MaxOnlinePreviewImageBytes)
            {
                return await File.ReadAllBytesAsync(cachedUri.LocalPath, cancellationToken);
            }
        }

        return await DownloadOnlineImageBytesWithRetryAsync(imageUri, cancellationToken);
    }

    private static async Task<(int Width, int Height)> TryReadOnlineImageDimensionsAsync(byte[] imageBytes)
    {
        try
        {
    ...
    private static string GetOnlinePreviewImageExtension(Uri imageUri, byte[] imageBytes)
    {
        ...
        if (imageBytes.Length >= 2 && imageBytes[0] == (byte)'B' && imageBytes[1] == (byte)'M')
        {
            return ".bmp";
        }
    ...
    private void ApplyLanguage()
    {
        ...
        ToolTipService.SetToolTip(OnlinePrevPageButton, null);
        ToolTipService.SetToolTip(OnlineNextPageButton, null);
    ...
    private void BuildShortcutRows()
    {
        ...
        {
            UpdateOnlineDetailDownloadControls(_activeOnlineDetailMod);
        }
        OnlineDetailRequirementTitleTextBlock.Text = L("可能存在额外访问要求", "Possible extra access requirements");
        ApplyShortcutPlaceholders();
        UpdateShortcutRowVisibility();
    ...
    private async void OnSelectOnlineDetailDownloadClicked(object sender, RoutedEventArgs e)
    {
        if (_activeOnlineDetailMod is null)
        {
            return;
        ...
        IReadOnlyList<OnlineDownloadCandidate> candidates)
    {
        ...
        OnlineDownloadCandidate? defaultFile = OnlineDownloadSelectionPolicy.SelectDefault(candidates);
        ListViewItem? defaultItem = null;
        foreach (OnlineDownloadCandidate candidate in candidates)
        {
            string fileName = string.IsNullOrWhiteSpace(candidate.FileName)
                ? L($"文件 {candidate.FileId}", $"File {candidate.FileId}")
                : candidate.FileName;
            ...
            {
                metadata.Add(L($"版本 {candidate.Version}", $"Version {candidate.Version}"));
            }
            if (candidate.FileSizeBytes > 0)
            {
                metadata.Add(FormatFileSize(candidate.FileSizeBytes));
            }
            if (candidate.AddedAt > DateTimeOffset.MinValue)
            {
                metadata.Add(candidate.AddedAt.ToString("yyyy-MM-dd HH:mm", CultureInfo.CurrentCulture));
            }
            metadata.Add(candidate.IsArchived ? L("历史归档", "Archived") : L("当前文件", "Current"));
            if (!candidate.IsSupportedArchive)
            {
                metadata.Add(L("不能自动解压", "Cannot auto-extract"));
            }
            ...
            var item = new ListViewItem
            {
                ...
                HorizontalContentAlignment = HorizontalAlignment.Stretch,
                IsEnabled = candidate.IsSupportedArchive
            };
            fileList.Items.Add(item);
            if (ReferenceEquals(candidate, defaultFile))
            {
                defaultItem = item;
            }
        }
        ...
        {
            Text = L(
                "默认已选中最新的当前压缩包。历史版本仍可手动选择。",
                "The newest current archive is selected by default. Archived versions remain available for manual selection."),
            Style = (Style)Application.Current.Resources["MutedTextStyle"],
            TextWrapping = TextWrapping.Wrap
        });
        ...
        var dialog = new ContentDialog
        {
            Title = L($"选择下载文件 · {mod.Title}", $"Select Download File · {mod.Title}"),
            Content = content,
            PrimaryButtonText = L("下载所选文件", "Download Selected"),
            CloseButtonText = L("取消", "Cancel"),
            DefaultButton = ContentDialogButton.Primary,
        ...
        fileList.SelectionChanged += (_, _) =>
            dialog.IsPrimaryButtonEnabled = fileList.SelectedItem is ListViewItem { IsEnabled: true };

        ContentDialogResult result = await dialog.ShowAsync();
        return result == ContentDialogResult.Primary
            && fileList.SelectedItem is ListViewItem { Tag: OnlineDownloadCandidate selected }
                ? selected
                : null;
    }
    ...
    private async void OnExternalResourceClicked(object sender, RoutedEventArgs e)
    {
        ...
        await OpenExternalUrlAsync(url, L("打开项目地址失败", "Failed to open project address"));
    }
    ...
    private IReadOnlyCollection<string> GetCharacterFolderAliases(OnlineModCard mod)
    {
        var aliases = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        void AddAlias(string? value)
        {
            if (!string.IsNullOrWhiteSpace(value) && !IsGenericOnlineCharacterName(value))
            {
                aliases.Add(value.Trim());
            }
        }

        AddAlias(mod.CharacterName);

        string normalizedCharacterName = NormalizeCharacterLookup(mod.CharacterName);
        OnlineCharacterOption? option = _onlineCharacterOptions.FirstOrDefault(item =>
            (!string.IsNullOrWhiteSpace(mod.CategoryId)
                && string.Equals(item.CategoryId, mod.CategoryId, StringComparison.OrdinalIgnoreCase))
            || string.Equals(NormalizeCharacterLookup(item.Key), normalizedCharacterName, StringComparison.OrdinalIgnoreCase)
        ...
        {
            AddAlias(option.Key);
            AddAlias(option.DisplayNameZh);
            AddAlias(option.DisplayNameEn);
        ...
        EndfieldCharacterInfo? endfieldCharacter = FindEndfieldCharacter(mod.CharacterName)
            ?? EndfieldCharacters.FirstOrDefault(character =>
                !string.IsNullOrWhiteSpace(mod.CategoryId)
                && string.Equals(
                    character.GameBananaCategoryId,
                    mod.CategoryId,
                    StringComparison.OrdinalIgnoreCase));
        if (endfieldCharacter is not null)
        {
            foreach (string alias in endfieldCharacter.AllNames)
            {
                AddAlias(alias);
            }
        }
    ...
    private async Task<string?> PickDownloadFolderAsync(OnlineModCard mod)
    {
        ...
        if (repository is not null && Directory.Exists(repository.SourcePath))
        {
            ...
            {
                bool isSuggested = !string.IsNullOrWhiteSpace(suggestedFolder)
                    && string.Equals(childFolder, suggestedFolder, StringComparison.OrdinalIgnoreCase);
                folderComboBox.Items.Add(new ComboBoxItem
                {
                    Content = isSuggested
                        ? L($"{Path.GetFileName(childFolder)}（推荐）", $"{Path.GetFileName(childFolder)} (Recommended)")
                        : Path.GetFileName(childFolder),
                    Tag = childFolder
                });
                if (isSuggested)
                {
                    suggestedIndex = folderComboBox.Items.Count - 1;
                }
            }

            folderComboBox.SelectedIndex = suggestedIndex;

            string characterDisplayName = GetOnlineCharacterDisplayName(mod.CharacterName);
            string destinationHint = suggestedFolder is null
                ? L(
                    $"未在当前仓库中找到与“{characterDisplayName}”明确匹配的角色文件夹，请确认下载位置。",
                    $"No folder in this repository could be matched confidently to “{characterDisplayName}”. Please confirm the destination.")
                : L(
                    $"已识别角色“{characterDisplayName}”，并预选角色文件夹：{Path.GetFileName(suggestedFolder)}。",
                    $"Recognized “{characterDisplayName}” and preselected: {Path.GetFileName(suggestedFolder)}.");
    ...
    private void LoadConfig()
    {
        ...
        try
        {
            string configText = ReadConfigurationTextWithRecovery(_configPath, IsValidLegacyConfiguration);
    ...
    private string ReadConfigurationTextWithRecovery(string filePath, Func<string, bool> validator)
    {
        try
        {
            ...
            if (!validator(text))
            {
                throw new InvalidDataException($"Invalid configuration file: {Path.GetFileName(filePath)}");
            }

            return text;
        }
        catch when (TryRestoreLatestConfigurationBackup(filePath, validator, out string restoredText))
        {
            StatusTextBlock.Text = L(
                $"配置 {Path.GetFileName(filePath)} 损坏，已自动恢复最近备份。",
                $"{Path.GetFileName(filePath)} was damaged and the latest backup was restored automatically.");
            return restoredText;
        }
    }

    private void WriteConfigurationTextWithBackup(string filePath, string content)
    {
        string directory = Path.GetDirectoryName(filePath) ?? AppContext.BaseDirectory;
        Directory.CreateDirectory(directory);

        if (File.Exists(filePath)
            && (!_lastConfigurationBackupUtc.TryGetValue(filePath, out DateTimeOffset lastBackup)
                || DateTimeOffset.UtcNow - lastBackup >= TimeSpan.FromMinutes(10)))
        {
            Directory.CreateDirectory(_configurationBackupPath);
            string backupName = $"{Path.GetFileName(filePath)}.{DateTimeOffset.UtcNow:yyyyMMdd-HHmmssfff}.bak";
            File.Copy(filePath, Path.Combine(_configurationBackupPath, backupName), overwrite: false);
            _lastConfigurationBackupUtc[filePath] = DateTimeOffset.UtcNow;
            PruneConfigurationBackups(filePath, keepCount: 10);
        }
    ...
    private bool TryRestoreLatestConfigurationBackup(
        string filePath,
        ...
        out string restoredText)
    {
        ...
        try
        {
            if (!Directory.Exists(_configurationBackupPath))
            {
                return false;
            }

            foreach (string backupPath in Directory.EnumerateFiles(
                         _configurationBackupPath,
                         Path.GetFileName(filePath) + ".*.bak",
                         SearchOption.TopDirectoryOnly)
                     .OrderByDescending(File.GetLastWriteTimeUtc))
            {
                string candidate = File.ReadAllText(backupPath, Encoding.UTF8);
                if (!validator(candidate))
                {
                    continue;
                }

                string restoreTemporaryPath = filePath + ".restore-" + Guid.NewGuid().ToString("N");
                File.WriteAllText(restoreTemporaryPath, candidate, new UTF8Encoding(false));
                File.Move(restoreTemporaryPath, filePath, overwrite: true);
    ...
    private void PruneConfigurationBackups(string filePath, int keepCount)
    {
        try
        {
            foreach (string staleBackup in Directory.EnumerateFiles(
                         _configurationBackupPath,
                         Path.GetFileName(filePath) + ".*.bak",
                         SearchOption.TopDirectoryOnly)
                     .OrderByDescending(File.GetLastWriteTimeUtc)
    ...
    private static bool IsValidShellConfiguration(string text)
    {
        try
        {
    ...
    private static bool IsValidLegacyConfiguration(string text)
    {
        return !string.IsNullOrWhiteSpace(text)
            && text.Split(["\r\n", "\n"], StringSplitOptions.RemoveEmptyEntries)
                .Any(line => line.StartsWith("source_dir=", StringComparison.Ordinal));
    }
...
public sealed record OnlineCharacterCatalogCacheSnapshot(
    List<OnlineCharacterOption> Characters,
...
public sealed class OnlineModCard
{
    ...
    public List<OnlineDownloadCandidate> DownloadFiles { get; set; } = [];
...
public sealed class OnlineRateLimitException : HttpRequestException
{
    public OnlineRateLimitException(TimeSpan? retryAfter)
        : base("GameBanana rate limit reached.", null, HttpStatusCode.TooManyRequests)
    {
